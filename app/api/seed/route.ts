...
export async function GET() {
  try {
    // Create admin user
    const adminExists = await queryDb('SELECT id FROM "User" WHERE email = $1', ['admin@example.com']);
    ...
    const employeeExists = await queryDb('SELECT id FROM "User" WHERE email = $1', ['employee@example.com']);
    ...
    const customerExists = await queryDb('SELECT id FROM "Customer" WHERE email = $1', ['customer@example.com']);
    ...
    if (count === 0) {
      const employee = await queryDb('SELECT id FROM "User" WHERE email = $1', ['employee@example.com']);
      const customer = await queryDb('SELECT id FROM "Customer" WHERE email = $1', ['customer@example.com']);

      if (employee.length > 0 && customer.length > 0) {
        const jobDate = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString();
        await runDb(
          'INSERT INTO "Job" (title, description, address, date, price, status, "customerId", "assignedToId") VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
